...
namespace AskMe.Services
{
	public class BingSpeechService
	{
		...
		public BingSpeechService(AuthenticationService authService, string os)
		{
		...
		public async Task<SpeechResult> RecognizeSpeechAsync(string filename)
		{
            ...
            string requestUri = GenerateRequestUri(Constants.BingSpeechRecognitionEndpoint);
			string accessToken = authenticationService.GetAccessToken();
			var response = await SendRequestAsync(fileStream, requestUri, accessToken, Constants.AudioContentType);
			var speechResults = JsonConvert.DeserializeObject<SpeechResults>(response);

			fileStream.Dispose();
            if (speechResults != null && speechResults.results != null)
                return speechResults.results.FirstOrDefault();
            return null;
        }

        public async Task<SpeechResult> RecognizeSpeechAsync(Stream stream)
        {
            if (string.IsNullOrWhiteSpace(authenticationService.GetAccessToken()))
            {
                await authenticationService.InitializeAsync();
        ...
        public async void AuthenticateSpeechAsync()
        {
            if (string.IsNullOrWhiteSpace(authenticationService.GetAccessToken()))
            {
                await authenticationService.InitializeAsync();
